...
public class qnaDBBean {
    ...
    public List<qnaDataBean> getMember(){
        Connection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        List<qnaDataBean> member1=null;
        ...
        try {
            ...
            if (rs.next()) {//해당 아이디에 대한 레코드가 존재
            	...
            	member1 = new ArrayList<qnaDataBean>();
            	do{  
            	  qnaDataBean member = new qnaDataBean();//데이터저장빈 객체생성
    ...
    //qna테이블에 글을 추가(inset문)<=writePro.jsp페이지에서 사용
    public void insertArticle(qnaDataBean article) 
            throws Exception {
        Connection conn = null;
        PreparedStatement pstmt = null;
		ResultSet rs = null;
        ...
        try {
            ...
            // 쿼리를 작성
            sql = "insert into qna(writer,email,subject,passwd,reg_date,";
	...
	public List<qnaDataBean> getArticles(int start, int end)
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            	"select * from qna order by ref desc, re_step asc limit ?,? ");
    ...
    public int updateArticle(qnaDataBean article)
        ...
        try {
			...
			pstmt = conn.prepareStatement(
            	"select passwd from qna where num = ?");
			...
			if(rs.next()){
			  ...
			  if(dbpasswd.equals(article.getPasswd())){
                sql="update qna set writer=?,email=?,subject=?,passwd=?";
